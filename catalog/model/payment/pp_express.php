...
class ModelPaymentPPExpress extends Model {
	...
	public function paymentRequestInfo() {
		...
		//[SB] Added order id to call
		$data['PAYMENTREQUEST_0_INVNUM'] = $this->session->data['order_id'];
		...
		foreach ($this->cart->getProducts() as $item) {
			...
			$item_total += number_format($item_price * $item['quantity'], 2, '.', ''); //[SB] Omited ',' for the thousand separated to prevent errorneous calculations
		...
		foreach ($total_data as $total_row) {
			if (!in_array($total_row['code'], array('total', 'sub_total'))) {
				if ($total_row['value'] != 0) {
					...
					$data['L_PAYMENTREQUEST_0_NAME' . $i] = strip_tags($total_row['title']);//[SB] Added strip_tags
					$data['L_PAYMENTREQUEST_0_AMT' . $i] = $this->currency->format($total_row['value'], false, false, false);
					$data['L_PAYMENTREQUEST_0_QTY' . $i] = 1;
					$item_total = number_format($item_total + $item_price, 2, '.', ''); //[SB] Omited ',' for the thousand separated to prevent errorneous calculations
