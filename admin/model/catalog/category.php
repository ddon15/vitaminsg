...
class ModelCatalogCategory extends Model {
	public function addCategory($data) {
		...
		foreach ($data['category_description'] as $language_id => $value) {
			/*CPT1 Added cptitle */
			$this->db->query("INSERT INTO " . DB_PREFIX . "category_description SET category_id = '" . (int)$category_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', cptitle = '" . $this->db->escape($value['cptitle']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'");
		}
