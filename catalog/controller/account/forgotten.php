...
class ControllerAccountForgotten extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			//[SB] Changed message format
			$message  = sprintf($this->language->get('text_greeting'), $this->config->get('config_name'), $this->request->post['email']) . "\n\n";
			$message .= $this->language->get('text_password');
			$message .= $password . "\n\n";
			$message .= sprintf($this->language->get('text_instructions'), $this->url->link('account/account', '', 'SSL'));
	...
	protected function validate() {
		//[SB] Changed to a better email validation
		if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
			$this->error['warning'] = $this->language->get('error_email_empty'); //[SB] added different error message for invalid
