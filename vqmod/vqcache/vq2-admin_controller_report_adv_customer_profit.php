...
if (isset($this->request->get['keepediting'])) {
		...
		$this->data['tax_classes'] = $this->model_report_adv_customer_profit->getOrderTaxClasses();					
		...
		$this->data['geo_zones'] = $this->model_report_adv_customer_profit->getOrderGeoZones();					
		...
		$this->data['coupon_names'] = $this->model_report_adv_customer_profit->getOrderCouponNames();
		...
		foreach ($results as $result) {
			...
			$this->data['customers'][] = array(	
				...
				'sales_total'      				=> $this->currency->format($total_sales_total, $this->config->get('config_currency')),
				'costs_total'      				=> $this->currency->format('-' . ($total_costs_total), $this->config->get('config_currency')),
				'profit_total'      			=> $this->currency->format(($total_sales_total-$total_costs_total), $this->config->get('config_currency')),
		...
		foreach ($sales_results as $sales_result) {
			$total_gsales = $sales_result['gsub_total']+$sales_result['ghandling']+$sales_result['glow_order_fee']+$sales_result['greward']+$sales_result['gcoupon']+$sales_result['gcredit']+$sales_result['gvoucher']+($this->data['adv_profit_reports_formula_cop1'] ? $sales_result['gshipping'] : 0);
			$total_gcosts = $sales_result['gprod_costs']+$sales_result['gcommission']+($this->data['adv_profit_reports_formula_cop3'] ? $sales_result['gpayment_cost'] : 0)+($this->data['adv_profit_reports_formula_cop2'] ? $sales_result['gshipping_cost'] : 0);
			...
			$this->data['sales'][] = array(
				...
				'gsales'      				=> $total_gsales,
				'gcosts'      				=> $total_gcosts,
				'gnetprofit'      			=> $total_gsales-$total_gcosts
			);
		}
	...
	public function coupon_autocomplete() {
		...
		$results = $this->model_report_adv_customer_profit->getCouponAutocomplete($data);
	...
	public function voucher_autocomplete() {
		...
		$results = $this->model_report_adv_customer_profit->getVoucherAutocomplete($data);
