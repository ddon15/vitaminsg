...
class ControllerCatalogReview extends Controller {
	...
	protected function getForm() {
		...
		$this->data['reward'] = $this->config->get('vit_review_rewards');
		...
		$this->data['entry_reward'] = $this->language->get('entry_reward'); //[SB] Added reward
		...
		//[SB] Added member link
		if($review_info['customer_id'] != 0) {
			$this->data['customer'] = '<a target="_blank" href="index.php?route=sale/customer/update&token='
				. $this->session->data['token'] . '&customer_id=' . $review_info['customer_id'] . '">' . $this->language->get('entry_customer') . '</a>';
		}
		else {
			$this->data['customer'] = '';
		}
		...
		$this->data['reward_total'] = $this->model_catalog_review->getTotalCustomerRewardsByReviewId($review_info['review_id']);
	...
	public function addReward() {
		...
		elseif (isset($this->request->get['review_id'])) {
			...
			$reward_total = $this->model_catalog_review->getTotalCustomerRewardsByReviewId($review_id);
			
			if (!$reward_total) {
				$review_info = $this->model_catalog_review->getReview($review_id);
				
				$this->model_catalog_review->addReward(
					$review_info['customer_id'],
					$review_info['product'],
					sprintf($this->language->get('text_reward_description'), $review_info['product']),
					$this->config->get('vit_review_rewards'),
					$review_id);
			...
			$json['error'] = $this->language->get('error_reward_unknown'); 
		}
		
		$this->response->setOutput(json_encode($json));
	}

	//[SB] Added removeReward
	public function removeReward() {
		...
		if (!$this->user->hasPermission('modify', 'catalog/review')) {
			...
			$this->load->model('catalog/review');
			$this->model_catalog_review->deleteReward($this->request->get['review_id']);
