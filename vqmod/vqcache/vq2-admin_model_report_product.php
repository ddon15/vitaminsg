...
class ModelReportProduct extends Model {
	...
	public function getProductsQuantity($data = array()) {
			$sql = "SELECT pd.name, p.model, p.quantity FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.quantity < 2 AND p.status = 1 ORDER BY p.quantity ASC, pd.name ASC LIMIT 999";	
			$query = $this->db->query($sql);
			return $query->rows;
		}
