...
class ControllerProductCategory extends Controller {  
	public function index() { 
		...
		if ($category_info) {
	  		/*CPT1 START*/if (!empty($category_info['cptitle'])){$this->document->setTitle($category_info['cptitle']);} else { $this->document->setTitle($category_info['name']);}/*CPT1 END*/
			...
			// [SB] Added check for categories
			if($this->model_catalog_category->hasCategoryFilters($category_id) &&
				empty($this->request->get['filter'])) {
				if(substr($category_info['name'], -1) == 's') {
					$category_name = substr($category_info['name'], 0, -1);
				}
				else {
					$category_name = $category_info['name'];
				}
				$this->data['text_filter_help'] = sprintf($this->language->get('text_filter_help'), $category_name);
			} else {
				...
				$swapimages = $this->model_catalog_product->getProductImages($result['product_id']);
					...
					if ($premium_member_price_data && (float)$premium_member_price_data['price'] && $result['price'] != 0) {
