...
class ControllerCatalogProduct extends Controller {
	private $error = array(); 
  	...
  	protected function getForm() {
		...
		$this->data['entry_redeem_only'] = $this->language->get('entry_redeem_only');
		...
		//[SB] Added Redemption
		if (isset($this->request->post['vit_redeem_only'])) {
			$this->data['redeem_only'] =
				($this->request->post['vit_redeem_only'] == 'vit_redeem_only');
		}
		elseif (!empty($product_info)) {
			$this->data['redeem_only'] = $product_info['redeem_only'];
		} else {
      		$this->data['redeem_only'] = false;
    	}
	...
	public function saveSkin() {
		 ...
		 $this->load->model('setting/setting');
			...
			//$this->request->post('schema')
				if (($this->request->server['REQUEST_METHOD'] == 'POST')) {	
				
		 $color_schema = $_POST['schema'];
 
    		 $issaveoption = $this->config->get('color_schema');

        if (!$issaveoption) {

            $options_jsonn = json_encode(array('0' => $color_schema));

//print_r($options_jsonn);
 			$this->model_setting_setting->editSetting('color_schema',array('color_schema'=>$options_jsonn ));
 
        }else {

            $old_json_data = json_decode($issaveoption, true);

            // array_push($old_json_data, $color_schema);
            $old_json_data[] = $color_schema;
		
				$this->model_setting_setting->editSetting('color_schema',array('color_schema'=>json_encode($old_json_data)) );		
		...
		public function LoadSkin() {
            ...
            if ($issaveoption) {
                    ...
                    $color_schema = json_decode($issaveoption, true);
                   // print_r($color_schema );
                          foreach ($color_schema as $key => $option):

                              if($key == $_POST['schema_id'] )  :
                    ...
                    $color_schema = json_decode($issaveoption, true);
                          ...
                          foreach ($color_schema as $key => $option):
                              ...
                              if($key == $_POST['schema_id'] )  :
                                  ...
                                  unset($color_schema[$key]);
                              
                                 $this->model_setting_setting->editSetting('color_schema',array('color_schema'=>json_encode($color_schema)) );		
