...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->get['search']) || isset($this->request->get['tag'])) {
			...
			//[SB]Add Additional Info
			if (isset($this->request->get['additional_info'])) {
				$url .= '&additional_info=' . $this->request->get['additional_info'];
			}
  ...
  //           $this->data['bulk_price'] = json_decode($bp['bulk_pricing'], true);
		...
		if ($product_info) {
			...
			/*CPT1 START*/if (!empty($product_info['cptitle'])){$this->document->setTitle($product_info['cptitle']);} else { $this->document->setTitle($product_info['name']);}/*CPT1 END*/
			...
			$this->data['text_reward'] = $this->config->get('vit_display_earn'); //[SB] Changed Vit$ Reward
			...
			$this->data['text_product_friend'] = $this->language->get('text_product_friend');
                        ...
                        //[MY]
                        $this->data['heading_brand'] = $this->language->get('heading_brand');
                        $this->data['heading_promotions'] = $this->language->get('heading_promotions');
			...
			//[SB] Added Redemption
			//$this->data['text_vit_dollar']  = $this->language->get('text_vit_dollar');
			$this->data['text_vit_dollar'] = $this->config->get('vit_display_redeem');
			
			$this->data['text_ask_a_question_title'] = $this->language->get('text_ask_a_question_title'); //[SB] Add Ask a Question
			$this->data['text_ask_a_question_description'] = $this->language->get('text_ask_a_question_description'); //[SB] Add Ask a Question
			...
			$this->data['entry_email'] = $this->language->get('entry_email'); //[SB] Add Ask a Question
			$this->data['entry_question'] = $this->language->get('entry_question'); //[SB] Add Ask a Question
			...
			$this->data['tab_ask_a_question'] = $this->language->get('tab_ask_a_question'); //[SB] Add Ask a Question
			...
			//[SB]Get member price
			$premium_member_price_data = $this->model_catalog_product->getPremiumMemberPrice($this->request->get['product_id']);
			if ($premium_member_price_data && (float)$premium_member_price_data['price']) {
				$this->data['premium_member_price'] = $this->currency->format($this->tax->calculate($premium_member_price_data['price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			    $this->data['premium_member_percent_savings'] = round((($product_info['price'] - $premium_member_price_data['price']) / $product_info['price'] * 100));
			} else {
				$this->data['premium_member_price'] = false;
			}
			$this->data['premium_member_display_name'] = $this->config->get('premium_member_display_name');
			
			//[SB] Added Redemption
			$this->data['points'] = $product_info['points'];
			$this->data['redeem_only'] = $product_info['redeem_only'];
            ...
            $this->data['original_price'] =  $this->currency->format($this->tax->calculate($product_info['original_price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
                        ...
                        $promotions_ava = $this->model_promotion_special_promotions->getPromotionByProduct($this->request->get['product_id']);
                        foreach ($promotions_ava as $promo) {
                            $promo_label = unserialize($promo['label']);
				$this->data['promotions'][] = array(
					'name' => $promo['name'],
                                        'id'=>$promo['promotion_id'],
                                        'label'=>$promo_label[$this->config->get('config_language_id')]
				);
                        }
                        /*
                        $this->load->model('promotion/db');

                        $product_promo = $this->model_promotion_db->getPromotionByProduct($this->request->get['product_id']);
                        
                        foreach ($product_promo as $promo) {
				$this->data['promotions'][] = array(
					'name' => $promo['name'],
                                        'id'=>$promo['promotion_id']
				);
                        ...
                        //[MY]check whether product is clearance
                        $products = explode(',', $this->config->get('clearance_product'));
                        $this->data['clearance'] = false;
                        if(in_array($product_id,$products)){
                            $this->data['clearance'] = true;
			...
			$this->data['can_write_review'] = $this->customer->isLogged(); //[SB] Added write review filter
			$this->data['reviews'] = sprintf($this->language->get('text_reviews'), (int)$product_info['reviews']);
			$this->data['rating'] = (int)$product_info['rating'];
			$this->data['description'] = html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8');
			//[SB] Add Additional Info
			$this->data['additional_info'] = html_entity_decode($product_info['additional_info'], ENT_QUOTES, 'UTF-8');
			...
			foreach ($results as $result) {
				...
				//[SB]Get member price
				$premium_member_price_result = $this->model_catalog_product->getPremiumMemberPrice($this->request->get['product_id']);
				if ((float)$premium_member_price_result['price']) {
					$premium_member_price = $this->currency->format($this->tax->calculate($premium_member_price_result['price'], $result['tax_class_id'], $this->config->get('config_tax')));
				} else {
				...
				$this->data['products'][] = array(
					...
					'points'	=> $product_info['points'], //[SB] Added Redemption
					'redeem_only'	=> $product_info['redeem_only'] //[SB] Added Redemption
				);
			}	
	...
	public function review() {
		...
		$this->data['text_review_disclaimer'] = $this->language->get('text_review_disclaimer');
		if(!$this->customer->isLogged()) {
			$this->data['text_signin_to_review'] = sprintf($this->language->get('text_signin_to_review'), $this->url->link('account/login', '', 'SSL'));
		} else {
			$this->data['text_signin_to_review'] = '';
		}
	...
	public function ask() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if ((utf8_strlen($this->request->post['ask-name']) < 3) || (utf8_strlen($this->request->post['ask-name']) > 25)) {
				$json['error'] = $this->language->get('error_ask_name');
			}
			
			if (!preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['ask-email'])) {
				$json['error'] = $this->language->get('error_ask_email');
			}
	
			if ((utf8_strlen($this->request->post['ask-text']) < 25) || (utf8_strlen($this->request->post['ask-text']) > 1000)) {
				$json['error'] = $this->language->get('error_ask_question');
			}
	
			if (empty($this->session->data['captcha']) || ($this->session->data['captcha'] != $this->request->post['ask-captcha'])) {
				$json['error'] = $this->language->get('error_captcha');
			}
			...
			if (!isset($json['error'])) {
				...
				$mail->setFrom($this->request->post['ask-email']);
				$mail->setSender($this->request->post['ask-name']);
				$mail->setSubject(sprintf($this->language->get('text_ask_subject'), $product_info['name']));
				$mail->setText(strip_tags(html_entity_decode($this->request->post['ask-text'], ENT_QUOTES, 'UTF-8')));
				$mail->send();
				
				$json['success'] = $this->language->get('text_ask_success');
			}
		}
