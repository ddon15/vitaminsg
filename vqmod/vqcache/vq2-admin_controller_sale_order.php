...
if (isset($this->request->get['keepediting'])) {
	...
	protected function getList() {
		...
		$data = array(
			...
			'filter_invoice' => $filter_invoice, //[SB] Added filter invoice

			'filter_customer'	     => $filter_customer,

			'filter_email' => $filter_email, //[SB] Added filter email
		...
		foreach ($results as $result) {
			...
			$this->data['orders'][] = array(
				...
				'invoice' => $result['invoice'], //[SB] Added invoice

				'customer'      => $result['customer'],

				'email'      => $result['email'], //[SB] Added email
		...
		$this->data['column_invoice'] = $this->language->get('column_invoice'); //[SB] Added Column Invoice

		$this->data['column_customer'] = $this->language->get('column_customer');

		$this->data['column_email'] = $this->language->get('column_email'); //[SB] Added Column Email
		...
		//[SB] Added sort invoice

		$this->data['sort_invoice'] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . '&sort=invoice' . $url, 'SSL');
		...
		$this->data['filter_invoice'] = $filter_invoice; //[SB] added filter invoice

		$this->data['filter_customer'] = $filter_customer;

		$this->data['filter_email'] = $filter_email; //[SB] added filter email
	...
	public function getForm() {
		...
		$this->data['entry_referral'] = $this->language->get('entry_referral'); //[SB] Added Referral
		...
		*/
		...
		$redeem_total = 0; //[SB] Added Redemption
		...
		foreach ($order_products as $order_product) {
			...
			if($order_product['redeem_total'] > 0) {

				$redeem_total += $order_product['redeem_total'];
			...
			$this->data['order_products'][] = array(
				...
				'unit_points'		=> (int)$order_product['redeem_price'], //[SB] Added Redemption. Caution, casted to int

				'total_points'		=> (int)$order_product['redeem_total'] //[SB] Added Redemption. Caution, casted to int
		...
		$this->data['text_redemption_price'] = $this->language->get('text_redemption_price');

		$this->data['text_redemption_total'] = $this->language->get('text_redemption_total');
		...
		$this->data['redeem_total'] = $redeem_total;
		...
		$referral_info = $this->model_sale_order->getOrderReferral($this->request->get['order_id']);

		if($referral_info) {

			$this->data['referrer_id'] = $referral_info['referrer_id'];

			$this->data['referrer_email'] = $referral_info['email'];

			$this->data['referrer_rewarded'] = $referral_info['rewarded'];

		}

		$this->data['text_referral_confirm'] = $this->language->get('text_referral_confirm');

		$this->data['text_referral_remove'] = $this->language->get('text_referral_remove');
...
//			}
	...
	public function info() {
		...
		$this->data['text_close'] = $this->language->get('text_close');	
			...
			foreach ($products as $product) {
				...
				if($product['redeem_total'] > 0) {

					$redeem_total += $product['redeem_total'];
				...
				$this->data['products'][] = array(
					...
					'unit_points'		=> (int)$product['redeem_price'], //[SB] Added Redemption. Caution, casted to int

					'total_points'		=> (int)$product['redeem_total'] //[SB] Added Redemption. Caution, casted to int
	...
	private function getAdditionalRewards($order_info) {
		...
		if($order_info['is_first_order'] &&

			(float)$order_info['total'] > (float)$vit_dollar_perks['vit_first_total_more_than'] &&
...

		{
			...
			$additional_perks[$this->language->get('text_rowhead_first_time')] =
		...
		if($order_info['is_first_order_of_dob'] &&

			(float)$vit_dollar_perks['vit_bd_rewards_multiple'] > 0)

		{

			$additional_perks[$this->language->get('text_rowhead_birthday')] =

				$order_info['reward'] * ($vit_dollar_perks['vit_bd_rewards_multiple'] - 1); //exclude initial $reward from multiple
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id']) {
				...
				if (!$reward_total) {
					...
					//[SB] Added additional reward. use float cast just in case

					$reward_additional = $this->getAdditionalRewards($order_info);

					$reward_added = 0;

					foreach($reward_additional as $reward_add) {

						$reward_added += $reward_add;

					}

					//[SB] Changed to use invoice for description

					$invoice = $order_info['invoice_prefix'] . sprintf('%04d', $order_info['invoice_no']);

					$this->model_sale_customer->addReward($order_info['customer_id'], $this->language->get('text_order_confirmation') . ' ' . $invoice, (float)$order_info['reward'] + (float)$reward_added, $this->request->get['order_id']);

					//$this->model_sale_customer->addReward($order_info['customer_id'], $this->language->get('text_order_id') . ' #' . $this->request->get['order_id'], (float)$order_info['reward'] + (float)$reward_additional, $this->request->get['order_id']);
	...
	public function confirmReferral() {
		...
		} elseif (isset($this->request->get['order_id']) && isset($this->request->get['referrer_id'])) {

			$this->load->model('sale/order');

			$this->model_sale_order->confirmOrderReferral($this->request->get['order_id'], $this->request->get['referrer_id']);

			$json['success'] = $this->language->get('text_referral_confirmed');
	...
	public function removeReferral() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			$this->model_sale_order->removeOrderReferral($this->request->get['order_id']);

			$json['success'] = $this->language->get('text_referral_removed');
