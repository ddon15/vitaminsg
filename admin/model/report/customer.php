...
class ModelReportCustomer extends Model {
	...
	public function getTotalPremiumCustomers($data = array()) {
		...
		$sql = "SELECT COUNT(DISTINCT c.customer_id) as total FROM oc_customer c, oc_premium_member pm WHERE c.customer_id = pm.customer_id AND pm.premium_member_id != '' AND c.customer_group_id = '2'";

		if (!empty($data['filter_date_from'])) {

			$sql .= " AND DATE(pm.expiry) >= '" . $this->db->escape($data['filter_date_from']) . "'";

		}

		if (!empty($data['filter_date_end'])) {

			$sql .= " AND DATE(pm.expiry) <= '" . $this->db->escape($data['filter_date_end']) . "'";
	...
	public function getPremiumCustomers($data = array()) {
		...
		$sql = "SELECT CONCAT(c.firstname, c.lastname) as name, c.email, cgd.name as customer_group, c.status, pm.expiry FROM oc_customer c, oc_customer_group_description cgd, oc_premium_member pm  WHERE c.customer_group_id = cgd.customer_group_id AND c.customer_group_id = '2' AND c.customer_id = pm.customer_id";
