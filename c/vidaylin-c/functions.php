...
function add_member_referral($member, $referrals) {
	...
	$stmt = $db->prepare(
		"INSERT INTO vidaylin_customers(name, email, mobile, address, dateadded) " .
	...
	foreach ($referrals as $referral) {
		...
		$stmt = $db->prepare(
			"INSERT INTO vidaylin_customer_referrals (hash, name, email, dateadded, memberid) " .
		...
		$message = str_replace("{{CUSTOMER_NAME}}", $member['mname'], $message);
		$to = $referral["email"];
...
function add_referral_claim($referral) {
	...
	$stmt = $db->prepare("UPDATE vidaylin_customer_referrals SET name = :name, email = :email, mobile = :mobile, address = :address WHERE hash = :hash");
	$params = array();
	$params[':hash'] = $referral['hash'];
	$params[':name'] = $referral['name'];
	$params[':email'] = $referral['email'];
...
function add_referral_paypal($referral) {
	...
	//update the referral
	$stmt = $db->prepare("UPDATE vidaylin_customer_referrals SET paypaltxnid = :paypaltxnid, dateclaimed = :dateclaimed WHERE hash = :hash");
	$params = array();
	$params[':hash'] = $referral['hash'];
	...
	if (!$member['referrerhash']) {
		$stmt = $db->prepare("UPDATE vidaylin_customers SET dateclaimed = :dateclaimed, referrerhash = :referrerhash WHERE id = :id");
		$params = array();
		$params[':id'] = $member['id'];
...
function get_referral_by_rhash($hash) {
	...
	$stmt = $db->prepare("SELECT * FROM vidaylin_customer_referrals WHERE hash = :hash");
	$params = array();
	$params[':hash'] = $hash;
	$stmt->execute($params);
...
function get_member_by_id($id) {
	...
	$stmt = $db->prepare("SELECT * FROM vidaylin_customers WHERE id = :id");
	$params = array();
	$params[':id'] = $id;
	$stmt->execute($params);
	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
function get_member_by_email($email) {
	...
	$stmt = $db->prepare("SELECT * FROM vidaylin_customers WHERE email = :email");
	$params = array();
	$params[':email'] = $email;
	$stmt->execute($params);
	$row = $stmt->fetchAll(PDO::FETCH_ASSOC);
