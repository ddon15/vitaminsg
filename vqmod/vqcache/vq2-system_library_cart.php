...
class Cart {
	...
	public function getProducts() {
		if (!$this->data) {
			//[SB] For redeem only points calc
			$redeem_points_required = 0;
			$prods = array();
			...
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$option_cost = 0; //[SB] Added for cost reporting
					$option_points = 0;
					$option_weight = 0;
					...
					foreach ($options as $product_option_id => $option_value) {
						...
						if ($option_query->num_rows) {
							if ($option_query->row['type'] == 'select' || $option_query->row['type'] == 'radio' || $option_query->row['type'] == 'image') {
								$option_value_query = $this->db->query("SELECT pov.option_value_id, ovd.name, pov.quantity, pov.subtract, pov.price, pov.price_prefix, pov.cost, pov.cost_prefix, pov.cost, pov.cost_prefix, pov.points, pov.points_prefix, pov.weight, pov.weight_prefix FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_option_value_id = '" . (int)$option_value . "' AND pov.product_option_id = '" . (int)$product_option_id . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");//[SB] Added pov.cost, pov.cost_prefix for cost reporting
								...
								if ($option_value_query->num_rows) {
									...
									$option_data[] = array(
										...
										'cost'                    => $option_value_query->row['cost'], //[SB] Added for cost reporting
										'cost_prefix'             => $option_value_query->row['cost_prefix'], //[SB] Added for cost reporting
								...
								$option_data[] = array(
									...
									'cost'                    => '', //[SB] Added for cost reporting
									'cost_prefix'             => '', //[SB] Added for cost reporting
									'points'                  => '',
									'points_prefix'           => '',								
									'weight'                  => '',
									'weight_prefix'           => ''
					...
					$usual_price = $product_query->row['price']; //[SB] Added usual price
					$cost = $product_query->row['cost']; //[SB] Added for cost reporting
					...
					$is_on_sale = ($is_on_sale_query->num_rows > 0);

					//[SB] Check if product is on special promo				
					$is_on_special_promo_query = $this->db->query("SELECT 1 FROM  ".DB_PREFIX ."sp_promotion sp, "
						 . DB_PREFIX . "sp_promotion_products spp WHERE spp.product_id = ". (int)$product_id . " "
						 . "AND sp.promotion_id = spp.promotion_id AND sp.status = '1'"
						 . " AND ((sp.date_start = '0000-00-00' OR sp.date_start < NOW()) AND (sp.date_end = '0000-00-00' OR sp.date_end > NOW())) LIMIT 1");
					...
					//if(!isset($this->session->data['reward']) || '0' == $this->session->data['reward']) {
        			...
        			'maximum'         => $product_query->row['maximum'],
						...
						'maximum'         => $product_query->row['maximum'], //[SB] Added for maximum_quantity vqmod
						'subtract'                  => $product_query->row['subtract'],
						'stock'                     => $stock,
						'usual_price'               => ($usual_price + $option_price), //[SB] Added usual price
						'is_on_sale'               => $is_on_sale, //[SB] Added is on sale
						'is_on_special_promo'      => $is_on_special_promo, //[SB] Added is on special promo
						'price'                     => ($price + $option_price),

						'cost'            => ($cost + $option_cost),
            
						'cost'            => ($cost + $option_cost),//[SB] Added for cost reporting
						'total'                     => ($price + $option_price) * $quantity,
						'usual_total'               => ($usual_price + $option_price) * $quantity, //[SB] Added usual price
						...
						'redeem_only'				=> $product_query->row['redeem_only'] //[SB] Added Redemption
					);
					
					//[SB] For redeem only points calc
					if($prods[$key]['redeem_only']) {
						$redeem_points_required += $prods[$key]['points'];
					}
			...
			//[SB] if Vit$ used and it is more than the redeem only items
			// do not give rewards
			if(isset($this->session->data['reward']) && $redeem_points_required < $this->session->data['reward']) {
	...
	public function isFirstOrder() {
		...
		$order_query = $this->db->query("SELECT 1 FROM `" . DB_PREFIX . "order` WHERE
			order_status_id != '7' 
			AND invoice_no != '0' 
			AND customer_id = '" . (int)$this->customer->getId() . "' 
			AND customer_group_id = (SELECT customer_group_id FROM `" . DB_PREFIX . "customer_group_description` WHERE name = 'Premium Member') LIMIT 1");

		return ($order_query->num_rows == 0);
	}
}
?>