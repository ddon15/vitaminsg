...
class ControllerDesignBanner extends Controller {
	...
	protected function getForm() {
		...
		foreach ($banner_images as $banner_image) {
			...
			if($date_start != '0000-00-00' && $date_start != null) {
				$date_start = date('Y-m-d', strtotime($banner_image['date_start']));
			}
			else {
				$date_start = '';
			}
			$date_end = $banner_image['date_end'];
			if($date_end != '0000-00-00' && $date_end != null) {
				$date_end = date('Y-m-d', strtotime($banner_image['date_end']));
			}
			else {
				$date_end = '';
			}
			...
			$this->data['banner_images'][] = array(
				...
				'image'                    => $image,
				'date_start'               => $date_start, //[SB] Add start/end date
				'date_end'                 => $date_end,
