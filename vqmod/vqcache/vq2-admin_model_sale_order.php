...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['order_total'])) {		
			foreach ($data['order_total'] as $order_total) {
                                ...
                                if ($order_total['code'] == 'special_promotions') {
                                    $order_total['title'] = html_entity_decode($order_total['title'], ENT_COMPAT, 'UTF-8');
                                    $order_total['text'] = html_entity_decode($order_total['text'], ENT_COMPAT, 'UTF-8');
                                }//.[MY]
	...
	public function editOrder($order_id, $data) {
		...
		if (isset($data['order_product'])) {
			foreach ($data['order_product'] as $order_product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_product_id = '" . (int)$order_product['order_product_id'] . "', order_id = '" . (int)$order_id . "', product_id = '" . (int)$order_product['product_id'] . "', name = '" . $this->db->escape($order_product['name']) . "', model = '" . $this->db->escape($order_product['model']) . "', quantity = '" . (int)$order_product['quantity'] . "', price = '" . (float)$order_product['price'] . "', total = '" . (float)$order_product['total'] . "', redeem_price = '" . (float)$order_product['redeem_price'] . "', redeem_total = '" . (float)$order_product['redeem_total'] . "', tax = '" . (float)$order_product['tax'] . "', reward = '" . (int)$order_product['reward'] . "', cost = '" . (float)$order_product['cost'] . "'");
	...
	//NOTE: Duplicated in /catalog/model/checkout/order.php
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			//[SB] Added First Time Perk reward
			$customer_id = $order_query->row['customer_id'];
			$is_first_order = ($order_id == $this->getFirstOrderId($customer_id));
			
			//[SB] Added Birthday Month reward
			$is_first_order_of_dob = ($order_id == $this->getFirstOrderIdOfDOB($customer_id));
			...
			return array(
				...
				'customer_id'             => $customer_id, //[SB] Used above variable instead
				...
				'reward'                  => $reward,
				'is_first_order'        => $is_first_order, //[SB] Added is_first_order
				'is_first_order_of_dob' => $is_first_order_of_dob, //[SB] Added is_first_order_of_dob
	...
	public function getOrders($data = array()) {
		//[SB] Added invoice, email
		$sql = "SELECT o.order_id,
		CASE WHEN o.invoice_no > 9999 THEN
			CONCAT(o.invoice_prefix, o.invoice_no)
		ELSE
			CONCAT(o.invoice_prefix, LPAD(o.invoice_no, 4, '0'))
		END AS 'invoice', 
		CONCAT(o.firstname, ' ', o.lastname) AS customer, email, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		//[SB] Added filter invoice
		if (!empty($data['filter_invoice'])) {
			$sql .= " AND (CONCAT(o.invoice_prefix, o.invoice_no) LIKE '%" . $this->db->escape($data['filter_invoice']) . "%' OR 
				CONCAT(o.invoice_prefix, LPAD(o.invoice_no, 4, '0')) LIKE '%" . $this->db->escape($data['filter_invoice']) . "%')";
		}
		...
		$sort_data = array(
			'o.order_id',
			'invoice', //[SB] Added sort invoice
			'customer',
			'email', //[SB] Added sort email
			'status',
			'o.date_added',
			'o.date_modified',
			'o.total'
	...
	//[SB] Added Referral
	public function getOrderReferral($order_id) {
		$query = $this->db->query("SELECT o.referrer_id, o.rewarded, c.email FROM `" . DB_PREFIX . "order_referral` o LEFT JOIN " . DB_PREFIX . "customer c ON (c.customer_id = o.referrer_id) WHERE o.order_id = '" . (int)$order_id . "'");
		
		return $query->row;
	}
	
	public function confirmOrderReferral($order_id, $referrer_id) {
	
		$order_info = $this->getOrder($order_id);
		$code = $this->config->get('vit_referral_prefix') . $order_info['invoice_prefix'] . sprintf('%04d', $order_info['invoice_no']);
		$dollar_off = $this->config->get('vit_referral_coupon_amount');
		$min_total = $this->config->get('vit_referral_coupon_min');
		$referral_reward = $this->config->get('vit_referral_reward');
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "coupon SET name = 'REFERRAL - " . $code . "', code = '" . $code . "', discount = '" . $dollar_off . "', type = 'F', total = '" . $min_total . "', logged = '0', shipping = '0', date_start = NOW(), date_end = '" . date('Y-m-d', strtotime('+1 year')) . "', uses_total = '1', uses_customer = '1', status = '1', date_added = NOW()");
		...
		$reward_id = $this->model_sale_customer->addReward($referrer_id, sprintf($this->language->get('description_referral_reward'), $order_info['firstname']), $referral_reward, $order_id);
		
		$this->db->query("UPDATE `" . DB_PREFIX . "order_referral` SET coupon_id = '" . $coupon_id . "', reward_id = '" . $reward_id . "', rewarded = '1'");

		$template = new Template();
		$template->data['title'] = sprintf($this->language->get('mail_referral_subject'), $order_info['store_name']);
		$template->data['logo'] = $order_info['store_url'] . 'image/' . $this->config->get('config_logo');		
		$template->data['store_name'] = $order_info['store_name'];
		$template->data['store_url'] = $order_info['store_url'];
		$template->data['text_address'] = $this->language->get('mail_address');
		$template->data['dear'] = sprintf($this->language->get('mail_dear'), html_entity_decode($order_info['firstname'], ENT_QUOTES, 'UTF-8'));
		$template->data['content'] = sprintf($this->language->get('mail_referral_text'),
			$order_info['store_name'], $dollar_off, $code, $min_total, $order_info['store_name']);
		
		$html = $template->fetch('mail/coupon_send.tpl');
	...
	public function removeOrderReferral($order_id) {
	
		$order_info = $this->getOrder($order_id);
		$code = $this->config->get('vit_referral_prefix') . $order_info['invoice_prefix'] . sprintf('%04d', $order_info['invoice_no']);
		
		$this->db->query("DELETE FROM " . DB_PREFIX . "coupon WHERE code = '" . $code . "'");
		
		$this->load->model('sale/customer');
		$reward_id = $this->model_sale_customer->deleteReward($order_id);
		
		$this->db->query("UPDATE `" . DB_PREFIX . "order_referral` SET coupon_id = NULL, reward_id = NULL, rewarded = '0'");
	}
}
?>